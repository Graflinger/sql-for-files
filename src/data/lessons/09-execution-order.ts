...
The grouped rows exist first, and then the window function ranks them.`,
    ...
    {
      id: "execution-order-window-functions-not-in-where",
      title: "Why Window Functions Cannot Go Directly in WHERE",
      content: `A common beginner question is: “If window functions run late, can I filter on them in \`WHERE\`?”
...
That works because the outer query's \`WHERE\` sees \`region_rank\` as a normal column from the inner query result.`,
      ...
      challenge: {
        ...
        hint:
          "First compute ROW_NUMBER() OVER (PARTITION BY region ORDER BY amount DESC) in an inner query, then filter the outer query to region_rank = 1.",
        initialSql: "-- Use a subquery or CTE to filter a window function\n",
        solutionSql:
          "WITH ranked_sales AS (\n  SELECT region, salesperson, amount,\n    ROW_NUMBER() OVER (\n      PARTITION BY region\n      ORDER BY amount DESC, id\n    ) AS region_rank\n  FROM sales\n)\nSELECT region, salesperson, amount, region_rank\nFROM ranked_sales\nWHERE region_rank = 1\nORDER BY region;",
        validate: (result) => {
          if (!hasColumns(result, ["region", "salesperson", "amount", "region_rank"])) {
            return {
              passed: false,
              message: "Return region, salesperson, amount, and region_rank.",
            };
          }
          ...
          if (result.rowCount !== 4) {
            return {
              passed: false,
              message: `Expected 4 top-per-region rows but got ${result.rowCount}.`,
            };
          }
          ...
          for (let index = 0; index < expected.length; index += 1) {
            const row = result.data[index];
            const [expectedRegion, expectedSalesperson, expectedAmount, expectedRank] = expected[index];
            const region = String(getValue(row, "region"));
            ...
            if (
              ...
              || regionRank !== expectedRank
            ) {
              return {
                passed: false,
                message: "Compute the window function first in an inner query, then filter the outer query to region_rank = 1 and order by region.",
              };
            }
          }
          ...
          return {
            passed: true,
            message: "Exactly. The subquery/CTE lets WHERE filter a window-function result after it has been computed.",
          };
        },
      },
    },
